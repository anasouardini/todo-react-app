...
const objMerge = (state, newState) => {
    const stateRef = {...state}; //shallow copy of the state, to throw of the silly react
...
/*
    ...
    ```js
    const objMerge = (state, newState) => {
        const stateRef = {...state};// state has references, I don't wanna deepClone it

        merger(stateRef, newState);//merges the two objects into the first one
